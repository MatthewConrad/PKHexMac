...
/// Data Tables for MemoryContext6
enum MemoryContext6Data {
    static let memoryNotXY: [UInt8] = [
    ...
    /// Memories for HMs ordered by HM index for bitflag checks
    static let moveSpecificMemoryHM: [UInt8] = [
    ...
    /// Locations with a PokeCenter in Kalos
    static let locationsWithPokeCenterXY: [UInt8] = [
        018, // Santalune City
        022, // Lumiose City
        030, // Camphrier Town
        040, // Cyllage City
    ...
    /// Loctaions with a PokeCenter in Hoenn
    static let locationsWithPokeCenterAO: [UInt8] = [
        172, // Oldale Town
        174, // Dewford Town
        176, // Lavaridge Town
        178, // Fallarbor Town
    ...
    static let memoryRandChance: [UInt8] = [
    ...
    /// Only item to show up as a memory arg that can be observed by other Pokemon in party
    static let keyItemUsableObserveEonFlute: UInt16 = 775

    static func isKeyItemMemoryArgValid(species: Species, arg: UInt16) -> Bool {
        switch species {
        case .Shaymin:
            return arg == 466
        case .Tornadus, .Thundurus, .Landorus:
            return arg == 638
        case .Kyurem:
    ...
    static let purchaseableItemXY: [UInt16] = [
    ...
    static let purchaseableItemAO: [UInt16] = [
    ...
    static let lotoPrizeXYAO: [UInt16] = [0001, 0033, 0050, 0051, 0053]

    /// Bigflag array with set bits indicating if the species ID can be captured in `GameVersion.X`
    static let captureFlagsX: [UInt8] = [
    ...
    /// Bigflag array with set bits indicating if the species ID can be captured in `GameVersion.Y`
    static let captureFlagsY: [UInt8] = [
    ...
    /// Bigflag array with set bits indicating if the species ID can be captured in `GameVersion.AS`
    static let captureFlagsAS: [UInt8] = [
    ...
    /// Bigflag array with set bits indicating if the species ID can be captured in `GameVersion.OR`
    static let captureFlagsOR: [UInt8] = [
        0x00, 0x40, 0x1A, 0x0A, 0xA8, 0x5E, 0x66, 0x85, 0x04, 0xAF, 0xD2, 0x81, 0x36, 0xA0, 0xF4, 0x81,
