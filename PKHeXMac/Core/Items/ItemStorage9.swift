...
struct ItemStorage9: IItemStorage {
    ...
    private static let validHeldTypes: [InventoryType] = [
        .Items, .TMHMs, .Medicine, .Berries, .Balls, .BattleItems, .Treasure,
    ]
    static let validTypes: [InventoryType] = validHeldTypes + [
        .KeyItems, .Ingredients, .Candy,
    ]
    
    static let allHeldItems = pouchOther + pouchTM + pouchMedicine + pouchBerries + pouchBall + pouchBall + pouchTreasure
    
    func getMaxCount(type: InventoryType) -> Int {
        return if type == .KeyItems {
            1
        } else if ItemStorage9.validTypes.contains(type) {
            999
        } else {
            0
        }
    ...
    func getItems(type: InventoryType) -> [Int] {
        ...
        case .KeyItems:
            return ItemStorage9.pouchEvent
        case .TMHMs:
            return ItemStorage9.pouchTM
        case .Medicine:
            return ItemStorage9.pouchMedicine
        case .Berries:
            return ItemStorage9.pouchBerries
        case .Balls:
            return ItemStorage9.pouchBall
        case .BattleItems:
            return ItemStorage9.pouchBattle
        case .Treasure:
            return ItemStorage9.pouchTreasure
        case .Ingredients:
            return ItemStorage9.pouchPicnic
        case .Candy:
            return ItemStorage9.pouchMaterial
        default:
            return []
        }
    }
